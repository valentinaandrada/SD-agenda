...
export class FormContactComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.editData) {
      ...
      this.contactForm.controls['favouriteColour'].setValue(
        this.editData.favouriteColour
      );
  ...
  addContact() {
    if (!this.editData) {
      const contact: Person = {
        ...
        birthday: this._contactService.formatDate(
  ...
  updateContact() {
    ...
    updatedContact.birthday = this._contactService.formatDate(
      this.contactForm.value.birthday
    );
